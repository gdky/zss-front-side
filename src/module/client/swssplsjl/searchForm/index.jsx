...
import {SelectorSPLX} from 'component/compSelector'
...
handleSubmit(e){
    ...
    } else if (form.getFieldValue('sqsj2')) {
       if (value.getTime() > form.getFieldValue('sqsj2').getTime() ) {
  ...
  disabledEndDate(rule, value, callback) {//日期校验规则方法
    const form = this.props.form;
    if (form.getFieldValue('sqsj')) {
       if (value&&value.getTime() < form.getFieldValue('sqsj').getTime() ) {
    ...
    render(){
      ...
      wrapperCol: { span: 20 },
        ...
        const clsj = getFieldProps('sqsj', {//设置日期输入组件校验规则
      rules: [
        { 
           type: 'date', 
         },{
       ...
       const clsj2 = getFieldProps('sqsj2', {//设置日期输入组件校验规则
      rules: [
        {
          type: 'date', 
           },
                ...
                <Row>
                    <Col span="8">
                        <FormItem
                          {...formItemLayout}
                          label="审批类型：">
                            <SelectorSPLX  {...getFieldProps('splx')}/>
                        </FormItem>
                    </Col>
                    <Col span="12">
                        <FormItem
